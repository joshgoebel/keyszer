...
REMAP_CORE_TERM = dict(
    Cmd = "RC",
    Opt = "Alt"
    # Ctrl = "Ctrl" # control is still control
)

SYS_MAP = REMAP_CORE | CONVENIENCE

def Mac(str):
    rewritten = [SYS_MAP.get(key, key) for key in str.split("-")]
    return K("-".join(rewritten))

# ─── LAUNCHER CLIENT VIA PIPES ────────────────────────────────────────────────

import os

LAUNCH_PIPE = "./launch.pipe"
try:
    os.mkfifo(LAUNCH_PIPE, 0o644)
...
define_keymap(re.compile(chromeStr, re.IGNORECASE),{
    K("C-comma"): [K("Alt-e"), usleep(10), K("s"),K("Enter")],    # Open preferences
    K("RC-q"):              K("Alt-F4"),              # Quit Chrome(s) browsers with Cmd+Q
    K("RC-y"):              K("C-h"),                   # access history
    ...
    K("RC-Left_Brace"):     K("Ctrl-page_up"),            # Page nav: Back to prior page in history
    K("BTN_SIDE"):     K("backspace"),            # Page nav: Back to prior page in history
    K("RC-Right_Brace"):    K("Ctrl-page_down"),           # Page nav: Forward to next page in history
}, "Chrome Browsers")
# Opera C-F12
...
define_keymap(lambda wm_class: wm_class.casefold() not in remotes,{
    ...
    # K("Alt-Key_2"):       [sleep(1), K("Shift-N"),K("o"),K("w"),K("Space"),K("i"),K("s"),K("Space"),K("t"),K("h"),K("e"),K("Space"),
...
define_keymap(re.compile(sublimeStr, re.IGNORECASE),{
    ...
    Mac("Ctrl-Space"): K("C-Space"),                    # Basic code completion
    Mac("Cmd-Ctrl-up"): K("Alt-o"),                       # Switch file
    Mac("Cmd-Ctrl-f"): K("f11"),                        # toggle_full_screen
    Mac("Cmd-Opt-v"): [K("C-k"), K("C-v")],             # paste_from_history
    Mac("Cmd-up"): pass_through_key,                    # cancel scroll_lines up
    Mac("Cmd-Opt-up"): K("C-up"),                       # scroll_lines up
    Mac("Cmd-down"): pass_through_key,                  # cancel scroll_lines down
    Mac("Cmd-Opt-down"): K("C-down"),                   # scroll_lines down
    Mac("Ctrl-Shift-up"): K("Alt-Shift-up"),            # multi-cursor up
    Mac("Ctrl-Shift-down"): K("Alt-Shift-down"),        # multi-cursor down
    Mac("Cmd-PAGE_DOWN"): pass_through_key,             # cancel next_view
    Mac("Cmd-PAGE_UP"): pass_through_key,               # cancel prev_view
    Mac("Cmd-Shift-left_brace"): K("C-PAGE_DOWN"),      # next_view
    Mac("Cmd-Shift-right_brace"): K("C-PAGE_UP"),       # prev_view
    Mac("Cmd-Opt-right"): K("C-PAGE_DOWN"),             # next_view
    Mac("Cmd-Opt-left"): K("C-PAGE_UP"),                # prev_view
    Mac("insert"): pass_through_key,                    # cancel toggle_overwrite
    Mac("Cmd-Opt-o"): K("insert"),                      # toggle_overwrite
    Mac("Alt-c"): pass_through_key,                     # cancel toggle_case_sensitive
    Mac("Cmd-Opt-c"): K("Alt-c"),                       # toggle_case_sensitive
    Mac("Cmd-h"): pass_through_key,                     # cancel replace
    Mac("Cmd-Opt-f"): K("C-h"),                         # replace
    Mac("Cmd-Shift-h"): pass_through_key,               # cancel replace_next
    Mac("Cmd-Opt-e"): K("C-Shift-h"),                   # replace_next
    Mac("f3"): pass_through_key,                        # cancel find_next
    Mac("Cmd-g"): K("f3"),                              # find_next
    Mac("Shift-f3"): pass_through_key,                  # cancel find_prev
    Mac("Cmd-Shift-g"): K("Shift-f3"),                  # find_prev
    Mac("Cmd-f3"): pass_through_key,                    # cancel find_under
    Mac("Ctrl-Alt-g"): K("C-f3"),                       # find_under
    Mac("Cmd-Shift-f3"): pass_through_key,              # cancel find_under_prev
    Mac("Ctrl-Alt-Shift-g"): K("C-Shift-f3"),           # find_under_prev
    Mac("Alt-f3"): pass_through_key,                    # Default - cancel find_all_under
    # Mac("Alt-Refresh"): pass_through_key,             # Chromebook/IBM - cancel find_all_under
    # Mac("Alt-C-g"): K("Alt-Refresh"),                 # Chromebook/IBM - find_all_under
    Mac("Cmd-Ctrl-g"): K("Alt-f3"),                     # Default - find_all_under
    Mac("Cmd-Shift-up"): pass_through_key,              # cancel swap_line_up
    Mac("Ctrl-Alt-up"): K("C-Shift-up"),                # swap_line_up
    Mac("Cmd-Shift-down"): pass_through_key,            # cancel swap_line_down
    Mac("Ctrl-Alt-down"): K("C-Shift-down"),            # swap_line_down
    Mac("Cmd-Pause"): pass_through_key,                 # cancel cancel_build
    Mac("Ctrl-c"): K("C-Pause"),                        # cancel_build
    Mac("f9"): pass_through_key,                        # cancel sort_lines case_s false
    Mac("f5"): K("f9"),                                 # sort_lines case_s false
    Mac("Ctrl-f9"): pass_through_key,                   # cancel sort_lines case_s true
    Mac("Ctrl-f5"): K("Super-f9"),                      # sort_lines case_s true
    Mac("Alt-Shift-1"): pass_through_key,               # cancel set_layout
    Mac("Cmd-Opt-1"): K("Alt-Shift-Key_1"),             # set_layout
    Mac("Alt-Shift-2"): pass_through_key,               # cancel set_layout
    Mac("Cmd-Opt-2"): K("Alt-Shift-Key_2"),             # set_layout
    Mac("Alt-Shift-3"): pass_through_key,               # cancel set_layout
    Mac("Cmd-Opt-3"): K("Alt-Shift-Key_3"),             # set_layout
    Mac("Alt-Shift-4"): pass_through_key,               # cancel set_layout
    Mac("Cmd-Opt-4"): K("Alt-Shift-Key_4"),             # set_layout
    Mac("Alt-Shift-8"): pass_through_key,               # cancel set_layout
    Mac("Cmd-Opt-Shift-2"): K("Alt-Shift-Key_8"),       # set_layout
    Mac("Alt-Shift-9"): pass_through_key,               # cancel set_layout
    Mac("Cmd-Opt-Shift-3"): K("Alt-Shift-Key_9"),       # set_layout
    Mac("Alt-Shift-5"): pass_through_key,               # cancel set_layout
    Mac("Cmd-Opt-Shift-5"): K("Alt-Shift-Key_5"),       # set_layout

    Mac("Ctrl-grave"): K("C-grave"),                    # toggle the console
    Mac("Cmd-Ctrl-Shift-f"): K("Shift-F11"),            # full screen / distraction free
    # Mac("Fn-f"): K("F11"),                              # full screen

    Mac("Cmd-Opt-f"): K("C-h"),                         # find/replace dialog
    Mac("Cmd-h"): pass_through_key,

    Mac("Cmd-0"): K("Alt-Key_0"),
    Mac("Cmd-1"): K("Alt-Key_1"),
    Mac("Cmd-2"): K("Alt-Key_2"),
    Mac("Cmd-3"): K("Alt-Key_3"),
    Mac("Cmd-4"): K("Alt-Key_4"),
    Mac("Cmd-5"): K("Alt-Key_5"),
    Mac("Cmd-6"): K("Alt-Key_6"),
    Mac("Cmd-7"): K("Alt-Key_7"),
    Mac("Cmd-8"): K("Alt-Key_8"),
    Mac("Cmd-9"): K("Alt-Key_9"),

    # Mac(""): pass_through_key,                    # cancel
    # Mac(""): K(""),                               #
}, "Sublime Text")
